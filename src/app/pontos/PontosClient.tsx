...
export default function PontosClient({ initialPontos, initialConfig }: { initialPontos: PontoTuristico[], initialConfig: { title: string, subtitle: string } }) {
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedCategory, setSelectedCategory] = useState('Todos');
  const [imageErrors, setImageErrors] = useState<Record<string, boolean>>({});
  const [pontos, setPontos] = useState<PontoTuristico[]>(initialPontos);
  ...
  const nextImage = useCallback(() => {
    if (selectedPonto?.images && selectedPonto.images.length > 0) {
      setActiveImageIndex((prev: number) => (prev + 1) % (selectedPonto.images?.length || 1));
    }
  }, [selectedPonto]);
