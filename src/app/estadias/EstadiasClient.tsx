...
export default function EstadiasClient({ initialHoteis, initialConfig }: { initialHoteis: Hotel[], initialConfig: { title: string, subtitle: string } }) {
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedCategory, setSelectedCategory] = useState('Todos');
  const [selectedHotel, setSelectedHotel] = useState<Hotel | null>(null);
  const [hoteis, setHoteis] = useState<Hotel[]>(initialHoteis);
              ...
              <div className="p-4 sm:p-6 flex flex-col gap-4 overflow-y-auto">
                ...
                <div className="mt-4 pt-4 border-t border-outline-variant/30 text-center">
                  <p className="font-sans text-[10px] text-on-surface-variant">
                    ...
                    <a 
                      href={`https://wa.me/5538992664400?text=Olá, sou o proprietário do estabelecimento *${selectedHotel.nome}* listado no Descubra Januária e gostaria de atualizar minhas informações ou reivindicar a página.`} 
                      target="_blank" 
