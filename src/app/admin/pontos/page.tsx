    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="columns-1 md:columns-2 lg:columns-3 gap-6 space-y-6">
          {filtered.map(item => (
            <div key={item.id} className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl shadow-sm overflow-hidden break-inside-avoid inline-block w-full">
              <div className="bg-gray-200 dark:bg-zinc-800 relative">
                {item.imagem ? <img src={item.imagem} alt={item.nome} className="w-full h-auto object-cover block" /> : <div className="w-full h-40 flex items-center justify-center text-gray-400 font-sans text-sm">Sem imagem</div>}
                <span className="absolute top-2 left-2 bg-green-500 text-white text-xs px-2 py-1 rounded font-sans font-bold shadow">{item.categoria}</span>
              </div>
              <div className="p-4">
              ...
              <div>
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                  ...
                  <div className="flex-1 w-full flex flex-col gap-2">
                    <input type="url" placeholder="Cole o link da imagem aqui..." value={form.imagem || ''} onChange={(e) => { setForm({ ...form, imagem: e.target.value }); setImagemFile(null); }} className={inputClass} />
