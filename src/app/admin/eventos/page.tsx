          ...
          {filtered.map(item => (
            <div key={item.id} className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl shadow-sm overflow-hidden break-inside-avoid inline-block w-full">
              <div className="bg-gray-200 dark:bg-zinc-800 relative">
                {item.imagem ? <img src={item.imagem} alt={item.nome} className="w-full h-auto object-cover block" /> : <div className="w-full h-40 flex items-center justify-center text-gray-400 dark:text-gray-500 font-sans text-sm">Sem imagem</div>}
                {item.destaque && <span className="absolute top-2 right-2 bg-purple-500 text-white text-xs px-2 py-1 rounded font-sans font-bold shadow">Destaque</span>}
              </div>
              <div className="p-4">
