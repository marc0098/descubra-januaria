...
export default function Dashboard() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-6 font-headline">Dashboard</h1>
      
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
        {cards.map((card) => (
          <div key={card.label} className="bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-xl shadow-sm p-6 transition-all duration-300">
            <div className={`${card.color} w-12 h-12 rounded-lg flex items-center justify-center mb-4`}>
              <card.icon className="w-6 h-6 text-white" />
            </div>
            <p className="text-3xl font-bold text-gray-900 dark:text-white font-sans">{card.value}</p>
            <p className="text-gray-500 dark:text-gray-400 mt-1 font-sans text-sm">{card.label}</p>
          </div>
        ))}
      </div>

      <div className="mt-8 bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-xl shadow-sm p-6 transition-all duration-300">
        <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4 font-headline">Bem-vindo ao Painel Admin</h2>
