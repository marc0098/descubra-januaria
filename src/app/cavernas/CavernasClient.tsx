...
export default function CavernasClient({ initialAtrativos, initialConfig }: { initialAtrativos: Atrativo[], initialConfig: { title: string, subtitle: string } }) {
  const [showMap, setShowMap] = useState(false);
  const [atrativos, setAtrativos] = useState<Atrativo[]>(initialAtrativos);
      ...
      <div className="max-w-[1300px] 2xl:max-w-[1600px] mx-auto px-4 sm:px-6 md:px-10 lg:px-16 xl:px-20 2xl:px-24 py-6 md:py-10 2xl:py-12">
        ...
        >
          ...
          <div className="mt-10 pt-10 border-t border-outline-variant/30 flex flex-col md:flex-row items-center gap-6">
            ...
            <Link href="/guias" className="whitespace-nowrap bg-primary text-white px-8 py-4 rounded-full font-sans text-xs font-bold uppercase tracking-widest hover:scale-105 transition-transform shadow-lg text-center">
              Agendar Visita
            </Link>
          </div>
        </motion.div>
